...
public class ExploCoopBehaviour extends SimpleBehaviour {
	...
	private int waitTime = 400;	// time between two actions
	...
	private boolean optimalSiloNodeVerbose = true;
	private boolean treasureVerbose = false;
	private boolean siloStrategyVerbose = true;
	...
	private int obstaclesExpirationTime = waitTimings ? waitTime*10 : 500;
	...
	private int considerSiloLostAfter = waitTimings ? 40000 : 4000;	// if we don't see the SILO for this long, we consider it lost
	...
	/*
	 * last time the SILO position was updated (updated only once every 20 seconds) */
	...
	private String getColor(String agentName, Boolean alternative) {
		if (agentName.equals("Tim") || agentName.equals("Agent1")) {
			if (!alternative) {
				...
				return "#FFE4E1"; // light salmon
			}
		} else if (agentName.equals("Elsa")) {
			if (!alternative) {
				return "#77FFFF"; // cyan
			} else {
				return "#DDFFFF"; // light sky blue
			}
...
//		}
	...
	private Behaviour checkForMapCommunication(String agentName) {
		...
		if (agentMap == null || this.myMap.equals(agentMap)) {
			return null;
		}
	 ...
	 */
	public String moveTowardsDestination(Location myPosition, String destination, List<String> obstacles, List<Couple<Location,List<Couple<Observation,String>>>> listObservations) {		
   ...
   private boolean attemptDepositTreasure(String siloName) {
      ...
      if (!emptySuccess) {
    	  if (treasureInteractionsVerbose) { 
    		  System.out.println(myAgent.getLocalName() + " - API call emptyMyBackPack failed for '" + siloName + "'. The agent is likely not a collector.");
    	  }
           return false;
      }
   ...
   private static class NodeScore implements Comparable<NodeScore> {
	    ...
	    public int compareTo(NodeScore other) {
	        ...
	        int scoreCompare = Float.compare(this.score, other.score);
	        if (scoreCompare != 0) {
	            return scoreCompare;
	        }
	        // Secondary sort: by node ID ascending (for consistent ordering of ties)
	...
	private String findOptimalSiloPosition() {
		...
		List<NodeScore> allNodeScores = new ArrayList<>();
		...
		for (String node : nodes) {
			...
			// 3.2.2) penalty for nodes with treasures
			TreasureInfo treasureAtNode = this.knownTreasures.get(node);
			if (treasureAtNode != null && treasureAtNode.amount > 0) {
				multiplier *= (1.0f + (float)treasureAtNode.amount * 10.0f); 
			}

			
			// 3.3) reduce the deadlocks
			// 3.3.1) get a node with a high number of edges			
			int numberOfEdges = neighborNodes.size(); // suppose > 0
			...
			if (optimalSiloNodeVerbose && this.myAgent.getLocalName().equals("Silo")) {
				allNodeScores.add(new NodeScore(node, score));
			}
		...
		if (optimalSiloNodeVerbose && !allNodeScores.isEmpty() && this.myAgent.getLocalName().equals("Silo")) {
	        allNodeScores.sort(null); 

	        System.out.println("\nSilo Position Scores (Sorted: ID:Score):");
	        final int ITEMS_PER_LINE = 5; // Adjust based on console width (multiply this by 14)
	        StringBuilder line = new StringBuilder();
	        for (int i = 0; i < allNodeScores.size(); i++) {
	        	// 5 lines max
	        	if (i >= 5 * ITEMS_PER_LINE) {
	        		break;
	        	}
	            NodeScore ns = allNodeScores.get(i);
	            String scoreStr;
	            if (ns.score >= 10000.0f) {
	                scoreStr = ">9999";
	            } else {
	                // Format for up to XXXX.YYY
	                scoreStr = String.format("%8.3f", ns.score).trim();
	            }
	            // Node ID (max 3c, left), Score String (max 8c, left)
	            String item = String.format("%-3.3s:%-8s", ns.id, scoreStr); 
	            line.append(String.format("%-14s", item)); // (3 + 1 + 8) + 2 = 14

	            if ((i + 1) % ITEMS_PER_LINE == 0 || i == allNodeScores.size() - 1) {
	                System.out.println(line.toString().trim()); // trim separator for last line
	                line.setLength(0);
	            } else {
	                line.append("| "); 
	            }
	        }
	        System.out.println("-- Best node: " + bestNodeFound + " with a score of " + bestScoreFound);
	        System.out.println("-- Current Silo pos: " + this.correctSiloPosition + " with status " + this.siloPositionStatus + ", live locationid: " + getPosition().getLocationId());
	    }
...
//		}
		...
		if (this.correctSiloPosition == null || !this.correctSiloPosition.equals(newBestPosition) || !this.correctSiloPosition.equals(getPosition().getLocationId())) {
			if (optimalSiloNodeVerbose) {
	...
	private void actionExploration(Location myPosition, List<Couple<Location,List<Couple<Observation,String>>>> listObservations, List<String> visibleAgents, List<String> obstacles) {
		...
		if (hasFinishedExploring() || noMovesLeft) {
			String reason = hasFinishedExploring() ? "know all nodes" : "can't find any accessible open nodes";
			if (isSiloAgent) {
				if (verbose) {
					System.out.println("\u001B[36m" + this.myAgent.getLocalName() + " - I " + reason + ", changing to phase 3" + "\u001B[0m");
				}
				phase = 3;
			}
			else {
				if (verbose) {
					System.out.println("\u001B[36m" + this.myAgent.getLocalName() + " - I " + reason + ", changing to phase 2" + "\u001B[0m");
				}
				phase = 2;
			}
			return;
	...
	private void setUpFindSiloMission() {
		...
		// testing: take the id=85 30% of the time, else random.
//		this.goalDestination = (Math.random() < 0.3) ? "85" : this.myMap.getRandomNode().getId();
		this.pathToDestination = null;
		...
		if (this.missionType.equals("pickUpTreasure")) {
			...
			if (closestNode.equals(myPosition)) {
                ...
                if (pickedAmount > 0 || (treasureInfo != null && treasureInfo.amount == 0 && treasureInfo.lockIsOpen)) {
                    ...
                    if (missionVerbose) {
                        System.out.println("\u001B[33m" + this.myAgent.getLocalName() + " - pickUpTreasure at " + myPosition + ": Failed to pick (e.g. no capacity, wrong type, incorrect position or already empty but knownTreasures not updated). Mission continues." + "\u001B[0m");
                    }
                }
			}
		}
...
//		}
		...
		// 6.3) check if we see the silo for findSilo mission
//		if ("findSilo".equals(this.missionType) && visibleAgents.contains("Silo")) {
//			System.out.println("findSilo mission and Silo is visible AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
//		    String actualSiloPosition = null;
//		    actualSiloPosition = getSiloPosition(); // This should be updated by ReceiveSiloKnowledge or direct obs
//		    System.out.println("actualSiloPosition : " + actualSiloPosition);
//
//		    if (actualSiloPosition != null && !actualSiloPosition.equals("-1")) {
//		        if (missionVerbose) {
//		            System.out.println("\u001B[32m" + this.myAgent.getLocalName() +  " - Found Silo at " + actualSiloPosition + " while on 'findSilo' mission (Silo is visible)! Transitioning." + "\u001B[0m");
//		        }
//		        this.lastSiloInteractionTimeStamp = System.currentTimeMillis();
//		        this.considerSiloLost = false;
//		        this.missionType = null; // This will trigger default mission setup next tick, hopefully goToSilo
//		        return;
//		    }
//		}

		
		// 6.4) try to move towards the destination (or temporary destination if there is one)
		String destination = (this.temporaryDestination != null) ? this.temporaryDestination : this.goalDestination;
		String moved = moveTowardsDestination(myPosition, destination, obstacles, listObservations);
		...
		case "onDestination":
			...
			if (this.goalDestination != null && this.goalDestination.equals(myPosition.getLocationId())) {
				...
				if (this.missionType != null) {
					...
					case "waitForSiloMessage":
						if (verbose) {
							System.out.println("\u001B[1m" + this.myAgent.getLocalName() + " - I am on the goal destination waiting for message [" + this.goalDestination + "]\u001B[0m");
						}
                        break;
					}
				}
				//this.isStationary = true;
				if (verbose) {
					if (this.missionType != "waitForSiloMessage") {
						System.out.println("\u001B[1m  " + this.myAgent.getLocalName() + " - I reached the destination [" + destination + "]\u001B[0m");
					}
				}
			}
			break;
		case "noPathFound":
			...
			this.isStationary = true;
			if (verbose) {
				System.out.println("\u001B[1m  " + this.myAgent.getLocalName() + " - I can't find a path to the destination [" + destination + "] <- [" + myPosition.getLocationId() + "]\u001B[0m");
			}
			break;
		case "moved":
		...
		case "failed":
			...
			if (verbose) {
				System.out.println("\u001B[1m  " + this.myAgent.getLocalName() + " - I failed to move to the destination [" + destination + "] <- [" + myPosition.getLocationId() + "]\u001B[0m");
			}
			break;
		case "golemInWay":
			...
			if (verbose) {
				System.out
						.println("\u001B[1m  " + this.myAgent.getLocalName() + " - A golem is in the way [" + destination + "] <- [" + myPosition.getLocationId() + "]\u001B[0m");
			}
			break;
		}
