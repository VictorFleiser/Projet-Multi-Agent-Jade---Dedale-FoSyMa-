...
import eu.su.mas.dedaleEtu.mas.agents.newAgents.ExploreCoopAgent5;
import eu.su.mas.dedaleEtu.mas.agents.newAgents.SiloAgent;
...
//		agentList.add(ag);	
		...
		List<String> list_golemNames=new ArrayList<String>();
//		for (int i = 1; i < 11; i++) {
//			list_agentNames.add("Agent" + i);
//		}
//		list_agentNames.add("Elsa");
//		list_agentNames.add("Tim");
//		list_agentNames.add("John");	// use resources/agent-3tests.json in configurationFile to use all 3 agents
//		// otherwise, comment the line above and use resources/agent-2collect.json
//		list_agentNames.add("Paul");
//		list_agentNames.add("Anna");
//		list_agentNames.add("Silo");
//
//		list_golemNames.add("Golem");
//		list_golemNames.add("Golem2");
//		list_golemNames.add("Golem3");
//		list_golemNames.add("Golem4");
//		list_golemNames.add("Golem5");

		
		list_agentNames.add("C1");
		list_agentNames.add("C2");
		list_agentNames.add("C3");
		list_agentNames.add("C4");
		list_agentNames.add("E1");
		list_agentNames.add("Silo");

		list_golemNames.add("G1");
		...
		for (int i = 0; i < list_agentNames.size(); i++) {
			...
			// entities file.
			agentName = list_agentNames.get(i);

			// 3) If you want to give specific parameters to your agent, add them here
			// copy list_agentNames except the current agent
			Object[] entityParametersExplo1 = new Object[list_agentNames.size() - 1];
			int j = 0;
			for (String agent : list_agentNames) {
				if (!agent.equals(agentName)) {
					entityParametersExplo1[j] = agent;
					j++;
				}
			}
			...
			if (agentName.equals("Silo")) {
				ag = createNewDedaleAgent(c, agentName, SiloAgent.class.getName(), entityParametersExplo1);
			} else {
				ag = createNewDedaleAgent(c, agentName, ExploreCoopAgent5.class.getName(), entityParametersExplo1);
			}
			agentList.add(ag);
		}
		
		for (int i = 0; i < list_golemNames.size(); i++) {
			// 1) Get the container where the agent will appear
			c = containerList.get(ConfigurationFile.LOCAL_CONTAINER2_NAME);
			Assert.assertNotNull("This container does not exist", c);
			...
			// 2) Give the name of your agent, MUST be the same as the one given in the
			// entities file.
			agentName = list_golemNames.get(i);

			// 3) If you want to give specific parameters to your agent, add them here
			// copy list_agentNames except the current agent
			Object[] entityParametersExplo1 = new Object[list_golemNames.size() - 1];
			int j = 0;
			for (String agent : list_golemNames) {
				if (!agent.equals(agentName)) {
					entityParametersExplo1[j] = agent;
					j++;
				}
			...
			// ag=createNewDedaleAgent(c, agentName,DummyMovingAgent.class.getName(),
			// entityParametersExplo1);
			ag = createNewDedaleAgent(c, agentName, DummyWumpusShift.class.getName(), entityParametersExplo1);
			agentList.add(ag);
		}
...
//		ag=createNewDedaleAgent(c, agentName, ExploreCoopAgent5.class.getName(), entityParametersExplo1);//ExploreSoloAgent
//		agentList.add(ag);
//
//		/*********
//		 * AGENT Explo2
...
//
//		//4) Give the class name of your agent to let the system instantiate it
//		ag=createNewDedaleAgent(c, agentName, ExploreCoopAgent5.class.getName(), entityParametersExplo2);//ExploreSoloAgent
//		agentList.add(ag);
//
//		//		
//		//		
...
//		
//		//3) If you want to give specific parameters to your agent, add them here
//		Object [] entityParametersExplo3={"John"};
//		
//		//4) Give the class name of your agent to let the system instantiate it
//		ag=createNewDedaleAgent(c, agentName, ExploreCoopAgent5.class.getName(), entityParametersExplo3);//ExploreSoloAgent
//		
//		agentList.add(ag);
//
		/*********
